        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <Card>
            <CardHeader>
              ...
              <CardDescription>The last 5 reports generated by the team.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {[
                  { date: '16 FEB 26', type: 'Daily', commander: 'Isiah KAYIRANGA', status: 'Final' },
                  { date: '15 FEB 26', type: 'Daily', commander: 'John DOE', status: 'Draft' },
                  { date: 'Week 07', type: 'Weekly', commander: 'Isiah KAYIRANGA', status: 'Final' },
                ].map((item, i) => (
                  <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-white border border-border/50 shadow-sm">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-muted rounded-md">
                        <FileText className="h-4 w-4 text-primary" />
                      </div>
                      <div>
                        <p className="font-medium text-sm">{item.type} Report - {item.date}</p>
                        <p className="text-xs text-muted-foreground">Commander: {item.commander}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${item.status === 'Final' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                        {item.status}
                      </span>
